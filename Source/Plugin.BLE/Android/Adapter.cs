...
public class Adapter : AdapterBase
{
	...
	public Adapter(BluetoothManager bluetoothManager)
	{
		...
		bondStatusBroadcastReceiver.BondStateChanged += (_, args) =>
		{
			...
			switch (args.State)
			{
				...
				case DeviceBondState.Bonded: tcs.TrySetResult(true);
					break;
			}
	...
	public override async Task<IDevice> ConnectToKnownDeviceNativeAsync(Guid deviceGuid, ConnectParameters connectParameters = default, CancellationToken cancellationToken = default)
	{
		...
		var nativeDevice = _bluetoothAdapter.GetRemoteDevice(macBytes) ?? throw new Abstractions.Exceptions.DeviceConnectionException(deviceGuid,"", $"[Adapter] Device {deviceGuid} not found.");
