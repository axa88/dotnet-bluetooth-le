...
public class Device : DeviceBase<BluetoothDevice>
{
	...
	{
		if (Gatt == null)
			Trace.Message($"[Warning]: Can't discover services {Name}. Gatt is null.");
			...
			.FromEvent<IReadOnlyList<IService>, EventHandler<ServicesDiscoveredCallbackEventArgs>, EventHandler>(
				execute: () =>
				{
					if (!Gatt.DiscoverServices())
						throw new("Could not start service discovery");
				},
				getCompleteHandler: (complete, reject) => (_, _) =>
				{
				...
				{
					reject(new($"Device {Name} disconnected while fetching services."));
				},
	...
	public override async Task<bool> UpdateRssiAsync()
	{
		...
		{
			Trace.Message($"You can't read the RSSI value for disconnected devices except on discovery on Android. Device is {State}");
			return false;
		}
		...
		return await TaskBuilder.FromEvent<bool, EventHandler<RssiReadCallbackEventArgs>, EventHandler>(
			...
			getCompleteHandler: (complete, reject) => (_, args) =>
			{
				if (args.Error == null)
				{
					Trace.Message($"Read RSSI for {Id} {Name}: {args.Rssi}");
					Rssi = args.Rssi;
					complete(true);
				}
				else
			...
			getRejectHandler: reject => (_, _) =>
			{
				reject(new($"Device {Name} disconnected while updating rssi."));
			},
			subscribeReject: handler => _gattCallback.ConnectionInterrupted += handler,
			unsubscribeReject: handler => _gattCallback.ConnectionInterrupted -= handler);
	}
	...
	protected override async Task<int> RequestMtuNativeAsync(int requestValue)
	{
		...
		return await TaskBuilder.FromEvent<int, EventHandler<MtuRequestCallbackEventArgs>, EventHandler>(
			...
			getRejectHandler: reject => (_, _) =>
			{
				reject(new($"Device {Name} disconnected while requesting MTU."));
			},
			subscribeReject: handler => _gattCallback.ConnectionInterrupted += handler,
			unsubscribeReject: handler => _gattCallback.ConnectionInterrupted -= handler
		);
